...
const FileTable = ({ folderId }) => {
  ...
  const handleDownload = async (fileName) => {
    ...
    try {
      ...
      const downloadUrl = `${API_BASE_URL}download_file?id=${folderId}&file=${fileName}`;
  ...
  const handleOpen = (fileName) => {
    const fileUrl = `${API_BASE_URL}login?final_doc_id=${folderId}&file=${fileName}`;
    window.open(fileUrl);
  };
  ...
  useEffect(() => {
    const fetchFiles = async () => {
      try {
        ...
        const response = await fetch(`${API_BASE_URL}list_files?id=${folderId}`);
